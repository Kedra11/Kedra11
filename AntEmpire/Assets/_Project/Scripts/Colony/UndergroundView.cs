...
namespace AntEmpire.Colony
{
    ...
    public class UndergroundView : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            // with each Queen Chamber level.
            float queenGrowth = 1f + 0.10f * rooms.GetLevel(RoomIds.QueenChamber);
            ...
            if (storageLevel > 0)
            {
                _foodChamber.localScale = SideChamberBaseScale * (1f + 0.08f * storageLevel);
                float fraction = (float)(game.ResourceManager.Get(ResourceType.Food) / rooms.FoodCapacity);
                _foodPile.localScale = Vector3.one * Mathf.Lerp(0.25f, 1.5f, Mathf.Clamp01(fraction));
            }
            ...
            if (nurseryLevel > 0)
            {
                _nurseryChamber.localScale = SideChamberBaseScale * (1f + 0.08f * nurseryLevel);
        ...
        private void Build()
        {
            ...
            AddPart(PrimitiveType.Cube, new Vector3(-2.2f, 3.8f, 0.4f), new Vector3(3.4f, 0.8f, 0.7f), cavity);
            AddPart(PrimitiveType.Cube, new Vector3(2.2f, 3.8f, 0.4f), new Vector3(3.4f, 0.8f, 0.7f), cavity);
            ...
            _foodChamber = AddPart(PrimitiveType.Sphere, new Vector3(-4.5f, 3.8f, 0.4f), SideChamberBaseScale, cavity, _foodGroup.transform);
            _foodPile = AddPart(PrimitiveType.Sphere, new Vector3(-4.5f, 3.45f, 0.3f), Vector3.one * 0.6f, foodColor, _foodGroup.transform);
            ...
            _nurseryChamber = AddPart(PrimitiveType.Sphere, new Vector3(4.5f, 3.8f, 0.4f), SideChamberBaseScale, cavity, _nurseryGroup.transform);
            AddPart(PrimitiveType.Sphere, new Vector3(3.9f, 3.5f, 0.3f), Vector3.one * 0.3f, eggColor, _nurseryGroup.transform);
            AddPart(PrimitiveType.Sphere, new Vector3(5.1f, 3.5f, 0.3f), Vector3.one * 0.3f, eggColor, _nurseryGroup.transform);
            _growingEgg = AddPart(PrimitiveType.Sphere, new Vector3(4.5f, 3.55f, 0.3f), Vector3.one * 0.15f, eggColor, _nurseryGroup.transform);
